...
module.exports = function(taskConfig, rootConfig) {
  ...
  var entries = {};
  if(taskConfig.entries) {
    entries = objectAssign({}, taskConfig.entries);
  }
  else {
    entries = glob.sync(taskConfig.entriesGlob || '**/*', {
      cwd: jsSrc,
      ignore: '**/_*.*'
    }).reduce(function(acc, cur) {
      acc[cur.split('.')[0]] = path.resolve(jsSrc, cur);
      return acc;
    }, {});
  }
  ...
  var wpconfig = {
    ...
    module: {
      loaders: createLoadersForGlobalPackages(globalModulePathToAliasMap).concat([
        ...
        },
        {
          test: /\.css$/,
          exclude: /node_modules/,
          loader: ExtractTextPlugin.extract('css' + cssQueryParams + '!postcss')
        },
        {
          test: /\.css$/,
        ...
        },
        {
          test: /\.scss$/,
          exclude: /node_modules/,
          loader: ExtractTextPlugin.extract('css' + cssQueryParams + '!postcss!sass')
        },
        {
          test: /\.scss$/,
          include: /node_modules/,
    ...
    sassLoader: objectAssign({}, taskConfig.sass, {includePaths: 'node_modules'}),
    postcss: [autoprefixer(taskConfig.autoprefixer)],
    resolve: {
      jsSrc,
