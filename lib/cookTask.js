...
var getFullSrc = function(config, rootConfig) {
  ...
  var srcVariants = baseSrcList.map(function(baseSrc) {
    ...
    var variants = [joinGlob(rootConfig.src, baseSrc)];

    if(config.npm) {
      variants.push(joinGlob(process.cwd(), 'node_modules', baseSrc));
    }
